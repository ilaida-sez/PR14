...
<body>
                            ...
                            <div class="specialty">
                                <div class="slider">
                                    <div class="inner">
                                        ...
                                        <div class="messages" id="messages-'.$read_news["id"].'">';
    ...
    <script>
        function SendMessage(sender) {
            let Message = sender.parentElement.children[0].value.trim();
            ...
            sender.parentElement.children[0].disabled = true;
            ...
            $.ajax({
                ...
                success: function (_data) {
                    sender.parentElement.children[0].disabled = false;
                    sender.disabled = false;
                    sender.parentElement.children[0].value = "";
                    
                    if(_data == "rate_limit") {
                        alert("Слишком часто. Подождите 3 секунды.");
                    }
                    else if(_data == "not_authorized") {
                        alert("Авторизуйтесь, чтобы комментировать.");
                    }
                    else if(_data == "empty") {
                        alert("Сообщение не может быть пустым.");
                    }
                    else if(_data == "too_long") {
                        alert("Сообщение слишком длинное.");
                    }
                    else if(_data == "ok") {
                        let messagesDiv = document.getElementById('messages-' + IdPost);
